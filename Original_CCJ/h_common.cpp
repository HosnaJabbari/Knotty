...
//    if (st->top < 10) {
//        (st->elem[st->top]).open = el->open;
//        (st->elem[st->top]).close = el->close;
//    }else{
//        fprintf(stderr,"bracket stack full, can't push!!\n");
//        exit(1);
//    }
}
...
//    result->open = (st->elem[st->top]).open;
//    result->close = (st->elem[st->top]).close;
//    st->top = st->top -1 ;
//    return result;
    return NULL;
}
...
void initNode(struct band_elem *head,int outer_end, int outer_start, int inner_end, int inner_start){
    ...
    head->outer_start=0; //this value is a dummy showing the root
...
// apending
void addNode(struct band_elem *head, int outer_end, int outer_start, int inner_end, int inner_start, char open, char close) {
//    band_elem *newNode = new band_elem;
//    newNode->outer_start = outer_start;
//    newNode->outer_end = outer_end;
//    newNode->inner_start = inner_start;
//    newNode->inner_end = inner_end;
//    newNode->open = open;
//    newNode->close = close;
//    newNode->next = NULL;
//    
...
void display(struct band_elem *head) {
//    band_elem *list = head;
//    while(list) {
//        printf(" outer_start = %d, outer_end =%d, inner_start =%d, outer_start = %d, open = %c and close =%c \n", head->outer_start, head->outer_start, head->inner_start, head->inner_end, head->open, head->close);
//        list = list->next;
//    }
}
